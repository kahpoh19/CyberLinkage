import { apiClient, withApiBase } from './client';
...
export const generateOutlineStream = async (
  ...
  const response = await fetch(withApiBase(`/api/projects/${projectId}/generate/outline/stream`), {
    method: 'POST',
    headers: {
...
export const generateDescriptionsStream = async (
  ...
  const response = await fetch(withApiBase(`/api/projects/${projectId}/generate/descriptions/stream`), {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(accessCode ? { 'X-Access-Code': accessCode } : {}),
