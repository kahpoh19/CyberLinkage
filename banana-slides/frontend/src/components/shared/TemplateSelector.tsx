...
import { getImageUrl, withAppBase } from '@/api/client';
            ...
            {presetTemplates.map((template) => (
              ...
              >
                {template.preview ? (
                  <>
                    <img
                      src={withAppBase(template.thumb || template.preview)}
...
export const getTemplateFile = async (
  ...
  if (presetTemplate && presetTemplate.preview) {
    try {
      const response = await fetch(withAppBase(presetTemplate.preview));
